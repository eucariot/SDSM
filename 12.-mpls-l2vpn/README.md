...
*   [VPLS][7]  
    *   [Data Plane][8]
    *   [Control Plane][9]
    *   [VPLS Martini Mode (targeted LDP)][10]  
        *   [Практика][11]
    *   [VPLS Kompella Mode (BGP)][12]  
        *   [Обнаружение соседей или Auto-Discovery][13]
        *   [Передача префиксов][14]
        *   [Распределение меток и механизм Label Block][15]
        *   [Практика][16]
*   [Иерархический VPLS (H-VPLS)][17]  
    *   [Практика H-VPLS][18]
*   [Проблемы VPLS][19]
*   [Полезные ссылки][20]
...
*   **Point-to-Point**. Применим к любым типам протоколов канального уровня и в принципе, в полной мере исчерпывает все сценарии применения L2VPN. Поддерживает все мыслимые и немыслимые протоколы. Причём некоторые из них ещё и по-разному [может реализовывать][21].  
...
**5.** На предпоследнем маршрутизаторе снимается транспортная метка — происходит [PHP][23]. На R6 пакет приходит с одной сервисной VPN-меткой.  
...
Если вы читали [предыдущий выпуск про L3VPN][24], то в вопросе передачи пользовательского трафика не увидите ничего нового — пара MPLS-меток и передача по транспортному LSP. Ingress PE проверяет какие метки поставить и в какой интерфейс отправить, P меняет транспортную метку, а Egress PE по метке VPN принимает решение, в какой AC-интерфейс передать полученный кадр.  
...
**Транспортная метка** может назначаться как LDP (см. [выпуск про MPLS][25]), так и RSVP-TE (ещё ждёт своего часа).  
...
> [UnetLab OVA][26].  
> [Cisco CSR1000V IOS-XE][27].  
> Инструкции по установке UNL и добавлению образов CSR1000V: [Тыц][28].  
>   
...
[![](https://habrastorage.org/files/092/3d4/7c1/0923d47c162d4e6b8732a6ae4d385d60.png)][29]  
  
Наша задача — прокинуть Ethernet от Linkmeup\_R1 (порт Gi3) до Linkmeup\_R4 (порт Gi3).  
  
На шаге **0** IP-адресация, IGP-маршрутизация и базовый MPLS уже настроены (см. [как][30]).  

> [Файл начальной конфигурации.][31]  
...
Соответственно ICMP-Reply возвращается только с меткой VPN, потому что на Linkmeup\_R2 возымел место [PHP][33] и транспортная метка была снята.  
...
Аналог [VRF/VPN-instance][35] в L3VPN.  
...
> Есть просто [гиф][36], которая показывает, что происходит.  
...
> “Virtual Private LAN Service (VPLS) Using BGP for Auto-discovery and Signaling”. [RFC 4761][37].  
> “Virtual Private LAN Service (VPLS) Using Label Distribution Protocol (LDP) Signaling” [RFC 4762][38].  
> [Историческая справка по методам.][39]  
...
VPLS Martini Mode описан в стандарте [RFC4762][40].  
...
> В конце [я расскажу][41] о других неприятных ситуациях, которые могут возникнуть в VPLS сети.  
...
[![](https://habrastorage.org/files/eab/9cc/5e0/eab9cc5e0b8d4fea892cf5b513764949.png)][42]  
_Кликабельно_.  
  

> Забыли о том, что у нас до этого был сервис VPWS — этой конфигурации больше нет.  
> [Файл начальной конфигурации.][43]  
...
> [Файл конфигурации VPLS Martini Mode][46].  
> 
...
> > Подробнее вы можете посмотреть в [альтернативном фaйле конфигурации VPLS Martini Mode][47].  
...
*   Для сигнализации меток VPN используется протокол LDP (метод [DU][48]).
...
*   Соседи на каждом узле настраиваются вручную. (Но к этому вопросу мы [ещё вернёмся][49] — всё совсем не так плохо).
...
Проблему поиска соседей решил Кирити Компелла — сотрудник Juniper. Он отталкивался от тех же критериев, но решил, что [MBGP][50], опробованный на L3VPN, подойдёт лучше на роль протокола распределения меток.  
...
В секции BGP для VPLS есть своя Address Family: L2VPN [AFI][51] (25) и VPLS [SAFI][52] (65)  
...
Если вы ещё не эволюционировали до понимания механизма Label Block, вернитесь к [видео четырьмя экранами выше][53].  
...
Скрупулёзное объяснение механизма Label Block от виновников: [Juniper][54].  
...
Знаете, что по этому поводу, говорит [RFC 4761][55]?  
...
И знаете, пока эта концепция блока меток не сформировалась в непротиворечивый набор синапсов в моём мозге, я с пренебрежением относился к ней. Она казалась мне топорной и устаревшей. Примерно, как [DVMRP][56]. Но теперь я проникся идеей и даже несколько удивлён тому, что внятного объяснения нигде нет.  
  
Замечу также, что ситуацию с потерянными метками несколько пересмотрели с выпуском [RFC 6624][57] (в котором, кстати, Компелла тоже принял непосредственное участие):  
...
И более того, режим [LDP-Signalling + BGP Auto-Discovery][58] позволяет совместить достоинства обоих методов. Хотя и появляется вот этот самый двухшаговый механизм — сначала изучаем соседей, потом рассылаем метки.  
...
> [Файл начальной конфигурации.][59]  
...
> [Файл конфигурации VPLS Kompella Mode][60].  
...
Командой **show bgp l2vpn vpls rd _X_ ve-id _Y_ block-offset _Z_** вы можете вывести всю информацию о блоке меток от данного соседа.  
...
Теория и практика VPLS Kompella mode на примере Juniper: [русским для русских][61].  
Конфигурация и примеры вычисления меток: [Сама cisco][62].  
...
Междоменные (Inter-AS) VPN не такая уж большая сложность для Martini при организации иерархического VPLS — тогда нет нужды создавать полносвязную топологию PE-устройств в VPLS-домене. Про H-VPLS мы ещё [поговорим позже][63]. Можем вычеркнуть это из его слабых сторон.  
...
**Во-вторых**, [RFC 4762][64] вводит механизм Auto-Discovery в режим Martini. Реализован он может быть через RADIUS или через тот же BGP.  
...
Можем. Наш путь к спасению — H-VPLS (Hierarchical VPLS), который описан в [RFC 4762][65].  
...
Другие названия PE-rs: **[PE-POP][66]**, **[n-PE][67]**.  
...
Другие названия MTU-s: **[u-PE][68]**, **[PE-CLE][69]**.  
...
> [Файл начальной конфигурации.][70]  
...
2.  На Linkmeup\_R1 создаём PW до Linkmeup\_R2.  
...
3.  Осталось терминировать эти PW на стороне Linkmeup\_R2:  
...
Немного приоткроют завесу над этими вопросами следующие два документа: [H-VPLS PE-rs Redundancy for QinQ Access][72] и [Настройка VPLS Multihoming на маршрутизаторах Juniper tutorial][73].  
...
Теперь представьте себе ситуацию, что приехал огромный такой трактор и начал точечно погружать свой ковш в землю пока не порвал оптику сразу в двух местах. На самом деле достаточно вспомнить про [плоские кольца][74], и ситуация уже не кажется такой невозможной.  
...
Имеется сеть VPLS с двумя центральными PE. Например, это сеть [DCI][75]. ДЦ1 подключен к двум PE — так называемый Dual-Homing — режиме Active/Active — то есть оба плеча могут принимать и передавать трафик. При этом коммутация внутри не происходит — поэтому петля исключена.  
...
*   Весело задорно про MPLS вообще: [Тыц][76].
*   Коротко о мирном AToM'е: [Тыц][77].
*   Описание работы Компелла:[Тыц][78].
*   Juniper о том, как работает механизм Label Block: [Тыц][79] и [Мыц][80].
*   Конфигурация VPLS BGP на Cisco и примеры вычисления меток: [Тыц][81].
*   Про Мартини от Мартини: [Тыц][82].
*   Скрупулёзное сравнение двух методов: [Тыц][83].
*   Историческая справка по методам: [Тыц][84].
*   Простыми английскими словами про H-VPLS: [Тыц][85].
*   H-VPLS от циски, как всегда сложно: [Тыц][86].
*   H-VPLS Multihoming и STP по-русски: [Тыц][87].
*   Внеклассное чтение — EVPN: [Тыц][88].

  
**Концептуальные вещи**  
✴ Luc De Ghein. [MPLS Fundamentals 1st Edition][89].  
✴ Ina Minei, Julian Lucek. [MPLS-Enabled Applications: Emerging Developments and New Technologies 3rd Edition][90].  
  
Также напоминаю, что список лучших книг по сетям сейчас [тут][91].  
Все аббревиатуры, использованные в статье, вы можете найти в [глоссарии linkmeup][92].  
Особо благодарных просим задержаться и пройти на Патреон.  
[![](http://simoncaine.co.uk/wp-content/uploads/2015/06/kaGh5_patreon_name_and_message.png)][93]  
  
**Благодарности**  
[Дмитрию Фиголю][94], Александру Клипперу и [Дмитрию JDima][95] за вычитку материала и комментарии.  
...
[2]: #VPWS
[3]: #VPWS-DATA-PLANE
[4]: #VPWS-CONTROL-PLANE
[5]: #VPWS-PRACTICE
[6]: #VPWS-TYPES
[7]: #VPLS
[8]: #VPLS-DATA-PLANE
[9]: #VPLS-CONTROL-PLANE
[10]: #VPLS-MARTINI-MODE
[11]: #VPLS-MARTINI-PRACTICE
[12]: #VPLS-KOMPELLA-MODE
[13]: #VPLS-KOMPELLA-AUTO-DISCOVERY
[14]: #VPLS-KOMPELLA-PREFIXES
[15]: #VPLS-KOMPELLA-LABEL-BLOCK
[16]: #VPLS-KOMPELLA-PRACTICE
[17]: #H-VPLS
[18]: #H-VPLS
[19]: #VPLS-WEAKNESS
[20]: #LINKS
[21]: https://www.ietf.org/rfc/rfc4717.txt
[22]: https://habrastorage.org/files/5e8/9d7/3e3/5e89d73e389245589d155cdef2304f22.png
[23]: http://lookmeup.linkmeup.ru/#term487
[24]: https://linkmeup.ru/blog/204.html
[25]: https://linkmeup.ru/blog/154.html
[26]: http://www.unetlab.com/download/index.html
