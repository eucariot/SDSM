...
>   
> Если вернуться к практическому [примеру по полисингу](http://linkmeup.ru/blog/365.html#POLICING-PRACTICE), где мы настраивали всплески \(Bc\), то станет понятно, что при большом значении \(1 875 000 байтов\) все всплески амортизируются хорошо. А при маленьком \(10 000 байтов\), несмотря на то, что оно заметно больше MTU, полисер упирается в постоянное заполнение ведра и отбрасывает большое количество пакетов.
...
Используется для PHB CS и EF, где превышения по скорости не ожидается, но если оно произошло, то лучше сразу отбросить.  
