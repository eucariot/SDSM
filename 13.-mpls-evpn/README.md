...
Да, спасибо [Марату Бабаяну][1]. EVPN станет вишенкой на торте MPLS L2VPN.  
...
*   [Базовая часть технологии EVPN][4]
*   [Лаборатория для тестов и конфигурации][5]
*   [Маршруты EVPN][6]  
    *   [Маршрут типа 3 (Inclusive Multicast Ethernet Tag Route)][7]
    *   [Маршрут типа 2 (MAC/IP Advertisement Route)][8]  
        *   [Изучение MAC-адресов][9]
    *   [Маршрут типа 1 (Ethernet Auto-Discovery Route)][10]  
        *   [Автоматический поиск multihomed PE и ESI label][11]
        *   [MAC Mass Withdrawal][12]
        *   [Aliasing label][13]
    *   [Маршрут типа 4 (Ethernet Segment Route)][14]  
        *   [Механизм выбора DF][15]
*   [L3-функционал в EVPN][16]  
    
    *   [IRB synchronisation][17]  
        [Маршрутизация между bridge-доменами][18]  
        [Выход в другие VRF и внешние сети][19]  
...
Думаю, что все уже прочитали [выпуск про L2VPN][22] и представляют себе, что такое VPLS и с чем его едят. Немного освежим в памяти какие виды VPLS существуют на сегодняшний день и чем же они существенно отличаются:  
...
Как и VPLS, EVPN использует для сигнализации исключительно протокол BGP, но использует уже новые NLRI: [AFI][23] 25 [SAFI][24] 70 (некоторые версии Wireshark еще не знают данное AFI/SAFI и при снятии дампа пишут unknown SAFI for AFI 25). Использование нового семейства адресов обусловлено тем, что EVPN использует для изучения MAC-адресов не только data-plane, как в стандартном VPLS или коммутаторе, но и control-plane:  
...
Для тестов я использовал [Unetlab][25], в которой собрал стенд из четырех [vMX][26] и трех Cisco IOL (L3). Как вы понимаете vMX-сы используется для эмуляции сети провайдера, а Cisco — как клиентские CE маршрутизаторы. Если кому интересно, то данная лаба была запущена на самом обычном ноутбуке с i5 и 12 Гб ОЗУ (из которых только 6 было занято, а загрузка CPU не превышала 30 процентов) — так что можете запустить у себя и пощупать EVPN.  
...
> Примечание: маршрут 5-го типа в настоящее время еще не утвержден в статуте RFC, и пока что описан только в [драфте][31] и поэтому в данной статье мы его рассматривать не будем.  
...
[![](https://habrastorage.org/files/4b0/a86/b2e/4b0a86b2eabe4a58824ec8ba2a3438a6.jpg)][32]  
...
[![](https://habrastorage.org/files/2cf/a64/e99/2cfa64e9929640fea06f60192338e64e.jpg)][33]  
...
После этого высчитывается номер DF по формуле [V mod N = i][34], где V — номер влана, а N количество PE маршрутизаторов в сегменте. Тот PE маршрутизатор, номер которого будет результатом вычисления и становится DF данного сегмента в данном влане.  
...
[![](https://habrastorage.org/files/6eb/d90/14f/6ebd9014f35c4499a3ea9b659eb837b5.JPG)][35]  
...
[![](https://habrastorage.org/files/90f/7a3/b3a/90f7a3b3a14549ccbdbc957556546a9e.JPG)][36]  
...
[![](https://habrastorage.org/files/501/b09/52c/501b0952cea04de492a17f83ffc0f50a.JPG)][37]  
...
[![](https://habrastorage.org/files/27b/e5c/680/27be5c680cb14fdf82bc8e9fc9dc446b.png)][38]  
...
[![](https://habrastorage.org/files/e5b/f16/5e9/e5bf165e9b4d4274aa9d05a2d165d21f.png)][39]  
...
[![](https://habrastorage.org/files/cea/9a1/535/cea9a15359634c2d8f61414e45d4812c.png)][40]  
...
1.  Непосредственно сам [RFC 7432][41] со всей историей изменений;
2.  Juniper [EVPN][42];
3.  Juniper [EVPN IRB][43];
4.  Cisco [EVPN PBB][44];
5.  Cisco [EVPN VXLAN][45];
6.  Brocade [EVPN][46].
...
[2]: https://en.wikipedia.org/wiki/Tier_2_network
[3]: #VPLS
[4]: #EVPN-BASIC
[5]: #EVPN-LAB
[6]: #ROUTE-TYPES
...
[20]: #WHYYYYYYY
[21]: #RESUME
[22]: https://linkmeup.ru/blog/261.html
[23]: http://lookmeup.linkmeup.ru/#term569
[24]: http://lookmeup.linkmeup.ru/#term568
[25]: http://www.unetlab.com/
[26]: http://www.juniper.net/us/en/products-services/routing/mx-series/vmx/
[27]: #VLAN-BASED
[28]: #VLAN-AWARE
[29]: https://en.wikipedia.org/wiki/Route_distinguisher
[30]: https://habrahabr.ru/sandbox/99255/
[31]: https://tools.ietf.org/html/draft-rabadan-l2vpn-evpn-prefix-advertisement-00
[32]: https://habrastorage.org/files/40a/ea9/bf2/40aea9bf28fb42629aaa383112532979.jpg
[33]: https://habrastorage.org/files/792/2b4/d0a/7922b4d0a9fe40b9a61c907bea12aa79.jpg
[34]: https://ru.wikipedia.org/wiki/%D0%94%D0%B5%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5_%D1%81_%D0%BE%D1%81%D1%82%D0%B0%D1%82%D0%BA%D0%BE%D0%BC
[35]: https://habrastorage.org/files/6eb/d90/14f/6ebd9014f35c4499a3ea9b659eb837b5.JPG
[36]: https://habrastorage.org/files/90f/7a3/b3a/90f7a3b3a14549ccbdbc957556546a9e.JPG
[37]: https://habrastorage.org/files/501/b09/52c/501b0952cea04de492a17f83ffc0f50a.JPG
[38]: https://habrastorage.org/files/27b/e5c/680/27be5c680cb14fdf82bc8e9fc9dc446b.png
[39]: https://habrastorage.org/files/e5b/f16/5e9/e5bf165e9b4d4274aa9d05a2d165d21f.png
[40]: https://habrastorage.org/files/cea/9a1/535/cea9a15359634c2d8f61414e45d4812c.png
[41]: https://datatracker.ietf.org/doc/rfc7432/
[42]: https://www.juniper.net/documentation/en_US/junos14.2/topics/concept/evpns-overview.html
[43]: https://www.juniper.net/documentation/en_US/junos14.1/topics/concept/evpn-irb-soultion-overview.html
[44]: http://www.cisco.com/c/en/us/products/collateral/routers/asr-9000-series-aggregation-services-routers/whitepaper_c11-731864.html
[45]: http://www.cisco.com/c/en/us/products/collateral/switches/nexus-9000-series-switches/guide-c07-734107.html
